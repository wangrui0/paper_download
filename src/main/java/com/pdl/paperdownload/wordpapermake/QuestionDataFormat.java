...
public class QuestionDataFormat {
  ...
  private static String IMAGE_PATH = QuestionDataFormat.class.getClassLoader().getResource("").getPath()
      .replace("classes", "tempfile");
  ...
  public static String getQuestionTypeCategory(Object questionTypeCategoryObj, Object qtIsSelectObj) {
    if (questionTypeCategoryObj != null) {
      ...
      if (!questionTypeCategory.equals(0)) {
        ...
        if (questionTypeCategoryStr != null) {
          return questionTypeCategoryStr;
        }
      }
    }
    if (qtIsSelectObj == null) {
      return null;
    }
    return " ";
  }
  ...
  public static Map<String, Object> htmlPaperToWordStyle(Map<String, Object> paperMap) {
    ...
    try {
      List<Map<String, Object>> questionsTypeList = JsonUtil
          .jsontoListMapObject(JsonUtil.toJson(paperMap.get("questionsTypeList")));
      for (Map<String, Object> qtMap : questionsTypeList) {
        List<Map<String, Object>> questionsList = JsonUtil
            .jsontoListMapObject(JsonUtil.toJson(qtMap.get("questionsList")));
        String questionTypeCategory = getQuestionTypeCategory(qtMap.get("questionTypeCategory"),
            qtMap.get("qtIsSelect"));//题型分类
