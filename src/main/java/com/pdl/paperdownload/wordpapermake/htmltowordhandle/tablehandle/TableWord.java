...
public class TableWord {
  ...
  public static String getWordTable(List<List<TableTd>> tableList) {
      ...
      for (tdIndex = 0; tdIndex < colnum; tdIndex++) {
        ...
        for (List<Integer> mergeList : mergeGroupList) {
          ...
          if (mergeCount > 0) {
            String tccolspan = "";
            if (mergeCount > 0) {
              tccolspan = tccolspanbegin + mergeCount + tccolspanend;
            }
    ...
    wordTableStr = wordTableStr.replace(
        "&LT;w:p w:rsidR=\"00752682\" w:rsidRDefault=\"00752682\"&GT;&LT;w:pPr&GT;&LT;w:jc w:val=\"center\"/&GT;&LT;/w:pPr&GT;&LT;/w:p&GT;",
        "");
